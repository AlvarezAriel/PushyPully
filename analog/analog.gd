...
func _ready():
	...
	print("Set up listenerNodePath: %s" %listenerNodePath)
	
	if (listenerNodePath != "" && listenerNodePath!=null):
		listenerNode = get_node(listenerNodePath)
		print("listenerNodePath=%s" % listenerNode)
	elif listenerNodePath=="":
		print("Analog controller was unable to find player")
...
func sendSignal2Listener():
	if (listenerNode != null):
		print("Sending signal: analog controller")
